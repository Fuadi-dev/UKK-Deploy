    ...
    <div class="container mx-auto px-3 lg:px-4 py-4 lg:py-8">
        ...
        <div class="bg-white rounded-2xl lg:rounded-3xl shadow-xl border border-gray-100 mb-4 lg:mb-8 overflow-hidden">
            <div class="bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-700 px-4 lg:px-8 py-4 lg:py-6">
                <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 lg:gap-0">
                    <div>
                        <h1 class="text-xl lg:text-3xl font-bold text-white mb-1 lg:mb-2">My Permission Requests</h1>
                        <p class="hidden lg:block text-purple-100">Submit and track your permission requests</p>
                    </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6 mb-4 lg:mb-8">
            <div class="bg-white rounded-xl lg:rounded-2xl shadow-lg border border-gray-100 p-3 lg:p-6">
                <div class="flex items-center">
                    <div class="w-8 h-8 lg:w-12 lg:h-12 bg-yellow-100 rounded-lg lg:rounded-xl flex items-center justify-center">
                    ...
                    <div class="ml-2 lg:ml-4">
                        ...
                        <p class="text-lg lg:text-2xl font-bold text-gray-800">{{ $permissions->where('status', 'pending')->count() }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-xl lg:rounded-2xl shadow-lg border border-gray-100 p-3 lg:p-6">
                <div class="flex items-center">
                    <div class="w-8 h-8 lg:w-12 lg:h-12 bg-green-100 rounded-lg lg:rounded-xl flex items-center justify-center">
                        <i class="fas fa-check text-green-600 text-sm lg:text-xl"></i>
                    </div>
                    <div class="ml-2 lg:ml-4">
                        <p class="text-gray-600 text-xs lg:text-sm">Approved</p>
                        <p class="text-lg lg:text-2xl font-bold text-gray-800">{{ $permissions->where('status', 'approved')->count() }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-xl lg:rounded-2xl shadow-lg border border-gray-100 p-3 lg:p-6">
                <div class="flex items-center">
                    <div class="w-8 h-8 lg:w-12 lg:h-12 bg-red-100 rounded-lg lg:rounded-xl flex items-center justify-center">
                        <i class="fas fa-times text-red-600 text-sm lg:text-xl"></i>
                    </div>
                    <div class="ml-2 lg:ml-4">
                        <p class="text-gray-600 text-xs lg:text-sm">Rejected</p>
                        <p class="text-lg lg:text-2xl font-bold text-gray-800">{{ $permissions->where('status', 'rejected')->count() }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-xl lg:rounded-2xl shadow-lg border border-gray-100 p-3 lg:p-6">
                <div class="flex items-center">
                    <div class="w-8 h-8 lg:w-12 lg:h-12 bg-blue-100 rounded-lg lg:rounded-xl flex items-center justify-center">
                        <i class="fas fa-calendar text-blue-600 text-sm lg:text-xl"></i>
                    </div>
                    <div class="ml-2 lg:ml-4">
                        <p class="text-gray-600 text-xs lg:text-sm">Total Hours</p>
                        <p class="text-lg lg:text-2xl font-bold text-gray-800">{{ $permissions->where('status', 'approved')->sum('duration_hours') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Permissions Table -->
        <div class="bg-white rounded-xl lg:rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <div class="px-4 lg:px-6 py-3 lg:py-4 border-b border-gray-200 bg-gray-50">
                <h3 class="text-base lg:text-lg font-semibold text-gray-800">Permission Requests</h3>
            </div>
                    ...
                    <div class="p-4 hover:bg-gray-50">
                        <div class="flex items-start gap-3 mb-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="font-medium text-gray-900 text-sm truncate">{{ str_replace('_', ' ', ucfirst($permission->permission_type)) }}</div>
                                @if($permission->project)
                                <div class="text-xs text-gray-500 truncate">{{ $permission->project->project_name }}</div>
                                @endif
                            </div>
                            <div>
                                @if($permission->status === 'pending')
                        ...
                        <div class="space-y-2 mb-3">
                            <div class="text-xs text-gray-600">
                                ...
                                {{ $permission->start_date->format('M d') }} - {{ $permission->end_date->format('M d, Y') }}
                            </div>
                            @if($permission->start_time)
                            <div class="text-xs text-gray-600">
                        ...
                        <div class="flex gap-2">
                            <button onclick="permissionManager.viewPermission({{ $permission->id }})" 
                                    class="flex-1 text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded-lg transition-colors text-xs font-medium">
                                <i class="fas fa-eye mr-1"></i>View
                            </button>
                            @if($permission->status === 'pending')
                            <button onclick="permissionManager.editPermission({{ $permission->id }})" 
                                    class="flex-1 text-green-600 hover:text-green-900 bg-green-50 hover:bg-green-100 px-3 py-2 rounded-lg transition-colors text-xs font-medium">
                                <i class="fas fa-edit mr-1"></i>Edit
                            </button>
                            <button onclick="permissionManager.deletePermission({{ $permission->id }})" 
                                    class="flex-1 text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 px-3 py-2 rounded-lg transition-colors text-xs font-medium">
                    ...
                    <div class="p-6">
                        @if(!$userCard)
                            ...
                            <div class="text-center">
                                ...
                                <h3 class="text-lg font-bold text-gray-800 mb-2">No Card Assigned</h3>
                                <p class="text-sm text-gray-600 mb-4">You need to have an assigned card before you can submit permission requests.</p>
                            </div>
                        @else
                            <!-- No Permission Requests State -->
                            <div class="text-center">
                                ...
                                <h3 class="text-lg font-bold text-gray-800 mb-2">No Requests Yet</h3>
                                <p class="text-sm text-gray-600 mb-4">Create your first request to get started.</p>
                                <button onclick="permissionManager.openCreateModal()" 
                                        class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all duration-200 shadow-lg text-sm">
