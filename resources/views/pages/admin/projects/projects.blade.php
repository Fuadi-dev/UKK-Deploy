    ...
    <div class="lg:ml-64">
        ...
        <header class="bg-white/80 backdrop-blur-lg shadow-sm border-b border-white/20 p-3 lg:p-4">
            <div class="flex items-center justify-between">
                ...
                <h2 class="md:hidden text-lg font-bold text-gray-800">Projects</h2>
                
                <!-- Header Actions -->
                <div class="flex items-center space-x-2 lg:space-x-4">
                    @if(auth()->user()->role === 'admin' || auth()->user()->role === 'leader')
                        <a href="{{ route('projects.create') }}" class="bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-3 py-2 lg:px-4 lg:py-2 rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all duration-200 flex items-center space-x-1 lg:space-x-2 shadow-lg">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                            </svg>
                            <span class="text-sm lg:text-base">New</span>
                        </a>
                    @endif
        ...
        <main class="p-3 lg:p-6">
            ...
            <div class="bg-white/80 backdrop-blur-lg rounded-2xl lg:rounded-3xl shadow-2xl border border-white/20 p-4 lg:p-8 mb-4 lg:mb-8">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 lg:gap-4">
                    <div class="bg-gradient-to-br from-emerald-50 to-teal-100 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-emerald-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-emerald-600 text-xs lg:text-sm font-medium">Active</p>
                                <p class="text-xl lg:text-2xl font-bold text-emerald-800">{{ $projects->where('status', 'active')->count() }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-amber-50 to-yellow-100 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-amber-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-amber-600 text-xs lg:text-sm font-medium">Hold</p>
                                <p class="text-xl lg:text-2xl font-bold text-amber-800">{{ $projects->where('status', 'on_hold')->count() }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-100 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-blue-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-blue-600 text-xs lg:text-sm font-medium">Done</p>
                                <p class="text-xl lg:text-2xl font-bold text-blue-800">{{ $projects->where('status', 'completed')->count() }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-red-50 to-rose-100 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-red-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-red-600 text-xs lg:text-sm font-medium">Cancel</p>
                                <p class="text-xl lg:text-2xl font-bold text-red-800">{{ $projects->where('status', 'cancelled')->count() }}</p>
                            </div>
                            <div class="w-8 h-8 lg:w-10 lg:h-10 bg-red-500 rounded-lg flex items-center justify-center">
                                <svg class="w-4 h-4 lg:w-5 lg:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="col-span-2 md:col-span-1 bg-gradient-to-br from-purple-50 to-pink-100 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-purple-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-purple-600 text-xs lg:text-sm font-medium">Total</p>
                                <p class="text-xl lg:text-2xl font-bold text-purple-800">{{ $projects->count() }}</p>
                            </div>
            ...
            <!-- Filter and Search -->
            <div class="bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg border border-white/20 p-3 lg:p-6 mb-4 lg:mb-8">
                <form method="GET" action="{{ route('projects') }}" class="flex flex-col gap-3 lg:gap-4">
                    <!-- Search -->
                    <div class="w-full">
                        <div class="relative">
                            <input type="text" name="search" value="{{ $search }}" placeholder="Search projects..." class="w-full pl-10 pr-4 py-2.5 lg:py-3 text-sm lg:text-base bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200">
                    ...
                    <div class="flex items-center gap-2 lg:gap-4">
                        <select name="sort_by" class="flex-1 px-3 py-2.5 lg:px-4 lg:py-3 text-sm lg:text-base bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            <option value="project_name" {{ $sortBy === 'project_name' ? 'selected' : '' }}>Name</option>
                        ...
                        <select name="sort_dir" class="flex-1 px-3 py-2.5 lg:px-4 lg:py-3 text-sm lg:text-base bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            <option value="asc" {{ $sortDir === 'asc' ? 'selected' : '' }}>Asc</option>
                            <option value="desc" {{ $sortDir === 'desc' ? 'selected' : '' }}>Desc</option>
                        </select>

                        <button type="submit" class="p-2.5 lg:p-3 bg-indigo-500 text-white border border-indigo-500 rounded-xl hover:bg-indigo-600 transition-colors flex-shrink-0">
            ...
            <!-- Projects Grid -->
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 lg:gap-8">
                @forelse($projects as $project)
                    <!-- Project Card -->
                    <div class="bg-white/80 backdrop-blur-lg rounded-2xl lg:rounded-3xl shadow-2xl border border-white/20 p-4 lg:p-6 hover:shadow-3xl lg:hover:scale-105 transition-all duration-300 group">
                        <div class="flex items-start justify-between mb-3 lg:mb-4">
                            <div class="w-10 h-10 lg:w-12 lg:h-12 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-xl lg:rounded-2xl flex items-center justify-center">
                                <span class="text-white font-bold text-base lg:text-lg">{{ strtoupper(substr($project->project_name, 0, 1)) }}</span>
                            </div>
                        ...
                        <h3 class="text-base lg:text-xl font-bold text-gray-800 mb-2 group-hover:text-indigo-600 transition-colors line-clamp-2">{{ $project->project_name }}</h3>
                        <p class="text-gray-600 text-xs lg:text-sm mb-3 lg:mb-4 leading-relaxed line-clamp-2 hidden md:block">{{ $project->description ?: 'No description available.' }}</p>
                        ...
                        <div class="space-y-2 lg:space-y-3 mb-4 lg:mb-6">
                            <div class="flex items-center justify-between text-xs lg:text-sm">
                                ...
                                <span class="font-medium text-gray-800 truncate ml-2">{{ $project->user->name }}</span>
                            </div>
                            ...
                            <div class="hidden lg:flex items-center justify-between text-xs lg:text-sm">
                        ...
                        <div class="flex items-center space-x-3">
                            <a href="{{ route('projects.show', $project->slug) }}" class="flex-1 bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-3 py-2 lg:px-4 lg:py-2 rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all duration-200 text-xs lg:text-sm font-medium text-center">
                                View Details
                            </a>
                        </div>
                    </div>
