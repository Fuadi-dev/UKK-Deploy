...
<body class="bg-gradient-to-br from-indigo-50 via-white to-purple-50 min-h-screen">
    ...
    <div class="lg:ml-64">
        ...
        <header class="bg-white/80 backdrop-blur-lg shadow-sm border-b border-white/20 p-3 lg:p-4">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <button id="addUserBtn" class="bg-gradient-to-r from-indigo-500 to-purple-600 text-white p-2 lg:px-4 lg:py-2 rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all duration-200 flex items-center space-x-2 shadow-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main class="p-3 lg:p-6">
            ...
            <div class="bg-white/80 backdrop-blur-lg rounded-2xl lg:rounded-3xl shadow-2xl border border-white/20 p-4 lg:p-8 mb-4 lg:mb-8">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
                    <div class="bg-gradient-to-br from-emerald-50 to-teal-100 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-emerald-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-emerald-600 text-xs lg:text-sm font-medium">Total Users</p>
                                <p class="text-xl lg:text-2xl font-bold text-emerald-800">{{ $users->total() }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-100 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-blue-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-blue-600 text-xs lg:text-sm font-medium">Admins</p>
                                <p class="text-xl lg:text-2xl font-bold text-blue-800">{{ $users->where('role', 'admin')->count() }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-amber-50 to-orange-100 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-amber-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-amber-600 text-xs lg:text-sm font-medium">Leaders</p>
                                <p class="text-xl lg:text-2xl font-bold text-amber-800">{{ $users->where('role', 'leader')->count() }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-rose-50 to-pink-100 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-rose-200">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xl lg:text-2xl font-bold text-rose-800">{{ $users->where('role', 'user')->count() }}</p>
                            </div>
            ...
            <!-- Filter and Search -->
            <div class="bg-white/80 backdrop-blur-lg rounded-xl lg:rounded-2xl shadow-lg border border-white/20 p-4 lg:p-6 mb-4 lg:mb-8">
                <form method="GET" action="{{ route('users') }}" class="flex flex-col gap-3 lg:gap-4">
                    <!-- Search -->
                    <div class="w-full">
                        <div class="relative">
                            <input type="text" name="search" value="{{ $search }}" placeholder="Search users..." class="w-full pl-10 pr-4 py-2.5 lg:py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 text-sm">
                    ...
                    <!-- Filters -->
                    <div class="grid grid-cols-2 lg:flex lg:items-center gap-2 lg:gap-4">
                        <select name="role" class="px-3 py-2.5 lg:px-4 lg:py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm">
                        ...
                        <select name="sort_by" class="px-3 py-2.5 lg:px-4 lg:py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm">
                        ...
                        <select name="sort_dir" class="px-3 py-2.5 lg:px-4 lg:py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm">
                            <option value="asc" {{ $sortDir === 'asc' ? 'selected' : '' }}>A-Z</option>
                            <option value="desc" {{ $sortDir === 'desc' ? 'selected' : '' }}>Z-A</option>
                        </select>

                        <button type="submit" class="col-span-2 lg:col-span-1 p-2.5 lg:p-3 bg-indigo-500 text-white border border-indigo-500 rounded-xl hover:bg-indigo-600 transition-colors">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                            <tr>
                                <th class="px-3 lg:px-6 py-3 lg:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">User</th>
                                <th class="px-3 lg:px-6 py-3 lg:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Role</th>
                                <th class="hidden md:table-cell px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                                <th class="hidden lg:table-cell px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Joined</th>
                                <th class="px-3 lg:px-6 py-3 lg:py-4 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200">
                            @forelse($users as $user)
                                <tr class="hover:bg-gray-50/50 transition-colors duration-200">
                                    <td class="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-2 lg:ml-4 min-w-0">
                                                <div class="text-xs lg:text-sm font-medium text-gray-900 truncate">{{ $user->name }}</div>
                                                <div class="text-xs lg:text-sm text-gray-500 truncate">{{ $user->email }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                                        <span class="px-2 lg:px-3 py-1 text-xs font-medium rounded-full capitalize
                                    ...
                                    <td class="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center space-x-1 lg:space-x-2">
                                            <button class="edit-user-btn p-1.5 lg:p-2 text-indigo-600 hover:text-indigo-900 hover:bg-indigo-50 rounded-lg transition-colors" 
                                            ...
                                            @if($user->id !== Auth::id())
                                                <form action="{{ route('users.destroy', $user) }}" method="POST" class="inline delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="delete-btn p-1.5 lg:p-2 text-red-600 hover:text-red-900 hover:bg-red-50 rounded-lg transition-colors">
                                                        <svg class="w-3.5 h-3.5 lg:w-4 lg:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
